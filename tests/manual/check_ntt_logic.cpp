...
struct finite_field_traits<Zq>
{
  ...
  static constexpr std::uint64_t field_order = P;
...
void check_field_arithmetic()
{
  ...
  if (c.data[0] == 0)
    std::println("Add Wrap (P-1 + 1): PASS");
  else
    std::println("Add Wrap (P-1 + 1): FAIL. Got {:x} (Expected 0)", c.data[0]);
  ...
  if (f.data[0] == P - 1)
    std::println("Sub Wrap (0 - 1): PASS");
  else
    std::println("Sub Wrap (0 - 1): FAIL. Got {:x}", f.data[0]);
  ...
  if (i.data[0] == 5)
    std::println("Add Assoc: PASS");
  else
    std::println("Add Assoc: FAIL. Got {:x}", i.data[0]);
  ...
  if (n.data[0] == 0)
    std::println("Add Commute: PASS");
  else
    std::println("Add Commute: FAIL");

  // Subtraction: 1 - (P-1) = 2
  Zq o = l - m;
  std::println("1 - (P-1): {:x} (Expected 2)", o.data[0]);
  if (o.data[0] == 2)
    std::println("Sub P-1: PASS");
  else
    std::println("Sub P-1: FAIL");
  ...
  std::println("k: {:x} (Expected {:x})", k.data[0], P - 2);
  if (k.data[0] == P - 2)
    std::println("Add Large: PASS");
  else
    std::println("Add Large: FAIL. Got {:x}", k.data[0]);
}
...
void check_multiplication()
{
  ...
  if (b.data[0] == 1)
    std::println("Mul NegOne: PASS");
  else
    std::println("Mul NegOne: FAIL");

  // (P-1) * 1 = P-1
  Zq c = a * Zq(1);
  if (c.data[0] == P - 1)
    std::println("Mul Identity: PASS");
  else
    std::println("Mul Identity: FAIL. Got {:x}", c.data[0]);
  ...
  if (e.data[0] == val - 1)
    std::println("Mul 2^64: PASS");
  else
    std::println("Mul 2^64: FAIL");
}
