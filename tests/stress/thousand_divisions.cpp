...
constexpr auto division_test()
{
  ...
  constexpr auto a_divided_by_b = lam::division_prototype<double, 4, a, 2, b>();
...
consteval auto divide_thousand_polynomials()
{
  ...
  using deg_unknown_poly = std::variant<deg_0_poly, deg_1_poly, deg_2_poly, deg_3_poly, deg_4_poly>;
  ...
  return std::make_pair(
    std::make_tuple(possible_deg_2_quotients, possible_deg_3_quotients, possible_deg_4_quotients, quotient_errors),
    std::make_tuple(possible_deg_0_remainders, possible_deg_1_remainders, possible_deg_2_remainders, remainder_errors));
}

int main()
{
  const std::string quotient_deg_2_file_string = filename + "2" + quotient_file_ext;
  const std::string quotient_deg_3_file_string = filename + "3" + quotient_file_ext;
  const std::string quotient_deg_4_file_string = filename + "4" + quotient_file_ext;
  const std::string remainder_deg_0_file_string = filename + "0" + remainder_file_ext;
  const std::string remainder_deg_1_file_string = filename + "1" + remainder_file_ext;
  const std::string remainder_deg_2_file_string = filename + "2" + remainder_file_ext;
  ...
  const auto remainders = quotients_and_remainders.second;
  const auto deg_0_remainders = std::get<0>(remainders);
  if (remainder_deg_0_file.is_open())
  {
    ...
    for (auto pair : deg_0_remainders)
    {
      ...
      auto r = pair.second;
      for (std::size_t i = 0; i <= lam::norm(r); ++i)
      {
        if (i != lam::norm(r))
