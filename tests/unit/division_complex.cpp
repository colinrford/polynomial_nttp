...
struct StructuralComplex 
{
    ...
    constexpr StructuralComplex operator+(const StructuralComplex& other) const {
        return {r + other.r, i + other.i};
    }
    constexpr StructuralComplex operator-(const StructuralComplex& other) const {
        return {r - other.r, i - other.i};
    }
    constexpr StructuralComplex operator-() const {
        return {-r, -i};
    }
    constexpr StructuralComplex operator*(const StructuralComplex& other) const {
    ...
    // needed for equality check in comparatore (fallback) and non-comparatore paths
    constexpr bool operator==(const StructuralComplex& other) const = default;
    
    // Construct from scalar (value_type) - implicit or explicit
    constexpr StructuralComplex(T v) : r(v), i(0) {}
    constexpr StructuralComplex(T re, T im) : r(re), i(im) {}
    constexpr StructuralComplex() : r(0), i(0) {}
    
    // Satisfy multiplicative_group_element_c_weak requirement for inversion/identity
    static constexpr StructuralComplex one() { return {static_cast<T>(1), static_cast<T>(0)}; }
};

// Helper helper for approx check of our mock
constexpr bool complex_approx_eq(StructuralComplex<double> a, StructuralComplex<double> b) {
    constexpr double tol = 1e-7;
    auto diff = a - b;
    auto real_diff = diff.r;
    auto imag_diff = diff.i;
    if (real_diff < 0.0) real_diff = -real_diff;
    if (imag_diff < 0.0) imag_diff = -imag_diff;
    return (real_diff < tol) && (imag_diff < tol);
}

consteval bool test_complex_division() 
{
    using Complex = StructuralComplex<double>;
    ...
    bool q_ok = complex_approx_eq(q[0], Complex{2.0, 0.0}) && 
                complex_approx_eq(q[1], Complex{1.0, 0.0});
